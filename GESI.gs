  ...
  BASE_URL = 'https://esi.tech.ccp.is/v'
     ...
     * @param {schematic_id} ID of the schematic to get information on.
     * @param {opt_headers} opt_headers Default: True, Boolean if column headings should be listed or not.
    ...
    function planetSchematic(schematic_id, opt_headers) {
        ...
        var response = UrlFetchApp.fetch(BASE_URL + endpoints.planetSchematic.version + endpoints.planetSchematic.url.replace("{schematic_id}", schematic_id));
        var response = JSON.parse(response);
